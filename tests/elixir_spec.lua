...
describe("elixir", function()
	describe("command", function()
		...
		it("returns false when it's not installed", function()
			...
			assert.are.same("tmp/fake_install/foo-bar_baz/foobarbaz/language_server.sh", tostring(result))
			assert.False(result:exists())
		end)

		it("returns true when it is installed", function()
			local install_path = "tmp/fake_install"
			shell("mkdir -p tmp/fake_install/foo-bar_baz/foobarbaz")
			shell("touch tmp/fake_install/foo-bar_baz/foobarbaz/language_server.sh")
